    ...
    <div className="container">
      ...
      <div className="card" style={{ padding: '0', overflowX: 'auto' }}>
        <table data-testid="transactions-table">
          ...
          <tbody>
            {transactions.map(t => (
              <tr key={t.id} data-testid="transaction-row" data-payment-id={t.id}>
                <td data-testid="payment-id" style={{ fontFamily: 'monospace' }}>{t.id}</td>
                <td data-testid="order-id" style={{ fontFamily: 'monospace' }}>{t.order_id}</td>
                <td data-testid="amount">₹{(t.amount / 100).toFixed(2)}</td>
                <td data-testid="method">{t.method}</td>
                <td>
                  <span
                    data-testid="status"
